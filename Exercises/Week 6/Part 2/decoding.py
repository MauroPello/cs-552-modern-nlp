...
class GreedySearchDecoderForCausalLM(GeneratorForCausalLM):
    ...
    def search(
        ...
        for t in range(max_new_tokens):
            ...
            # Greedy: pick the argmax token
            new_token_id = int(torch.argmax(next_token_logits, dim=-1).item())
        ...
        for t in range(max_new_tokens):
            ...
            for b in beams:
                ...
                if b["model_inputs"]["input_ids"][0][-1] == self.eos_token_id:
                    # finished beam stays as a candidate
                    candidates.append(b)
                else:
                    ...
                    topk_log_probs, topk_tokens = torch.topk(log_probs, num_beams, dim=-1)

                    # Loop over top k=num_beams tokens, build their next input and calculate their score
                    for log_prob, new_token_id in zip(topk_log_probs[0], topk_tokens[0]):
                        new_model_inputs = self.prepare_next_inputs(
                            model_inputs = b["model_inputs"],
                            new_token_id = int(new_token_id.item())
                        )
                        new_beam = {
                            'score': b['score'] + float(log_prob.item()),
                            'length': b['length'] + 1.0,
